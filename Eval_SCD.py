...
def Eval():
    ...
    INFER_DIR1 = '/data2/jw/semantic_cd/Bi-SRNet-main/PRED_Daudt4/im1/'  # Inference path1
    INFER_DIR2 = '/data2/jw/semantic_cd/Bi-SRNet-main/PRED_Daudt4/im2/'  # Inference path2
    LABEL_DIR1 = '/data2/jw/semantic_cd/datasets/JL1_second/test/label1_gray/'  # GroundTruth path
    LABEL_DIR2 = '/data2/jw/semantic_cd/datasets/JL1_second/test/label2_gray/'  # GroundTruth path

    infer_list1 = glob.glob(INFER_DIR1 + "*{}".format('.tif'))
    infer_list2 = glob.glob(INFER_DIR2 + "*{}".format('.tif'))
    ...
    assert set([os.path.basename(label).split(".")[0] for label in label_list1]) == set([os.path.basename(infer).split(".")[0] for infer in infer_list1]), "Predictions do not match targets name"
    assert set([os.path.basename(label).split(".")[0]  for label in label_list2]) == set([os.path.basename(infer).split(".")[0]  for infer in infer_list2]), "Predictions do not match targets name"
    ...
    print('OA:',(c2hist[1][1]+c2hist[0][0])/(c2hist[1][1]+c2hist[0][0]+c2hist[1][0]+c2hist[0][1]))
    hist_n0 = hist.copy()
    hist_n0[0][0] = 0
