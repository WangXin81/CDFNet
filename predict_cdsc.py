...
class PredOptions():
    ...
    def initialize(self, parser):
        ...
        parser.add_argument('--test_dir', required=False, default='datasets/JL11_PRED/', help='directory to test images')
        parser.add_argument('--pred_dir', required=False, default='./res', help='directory to output masks')
        parser.add_argument('--chkpt_path', required=False, default='checkpoints/JL1_222/cdsc_sca_2e_mIoU65.94_Sek19.36_Fscd62.54_OA82.53.pth')
...
def predict(net, pred_set, pred_loader, pred_dir, flip=False, index_map=False, intermediate=False):
    ...
    for vi, data in enumerate(pred_loader):
        ...
        # save_path = os.path.join(pred_dir,"change", mask_name)
        # print(mask_name)
        # cv2.imwrite(save_path,RS.Index2Color(JL1_mask))
        pred_A = RS.Index2Color(pred_A)
        pred_B = RS.Index2Color(pred_B)
        save_path_a = os.path.join(pred_dir, "im1", mask_name)
        save_path_b = os.path.join(pred_dir, "im2", mask_name)
        cv2.imwrite(save_path_a,pred_A)
        cv2.imwrite(save_path_b,pred_B)
